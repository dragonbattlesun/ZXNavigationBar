...
[![Support](https://img.shields.io/badge/support-iOS%2016.0%2B%20-blue.svg?style=flat)](https://www.apple.com/nl/ios/)&nbsp;
## 安装
### 通过CocoaPods安装
```ruby
pod 'ZXNavigationBar'
...
- [x] 支持 iOS 16.0 及以上版本，兼容刘海屏、药丸屏、iPad 和 iPhone Duo，并适配横竖屏与窗口尺寸变化
...
- [x] 支持自定义手势返回范围
- [x] 支持监听手势返回进度
- [x] 仿系统导航栏历史堆栈效果，支持自定义基本样式
- [x] 若从Xib中加载控制器View，添加子View无需手动设置距离导航栏顶部约束，`ZXNavigationBar`会自动处理

### iPhone Duo、旋转与多窗口

- 现有公开调用方式无需迁移；默认导航栏 frame 会以当前控制器的 `view.bounds` 为准，并随旋转、容器 resize 与多窗口尺寸变化重新收敛。
- 在 iOS 27.1 中，ZX 自定义导航栏使用 horizontal fallback（`UIVerticalBarBehaviorDisabled`），切换到系统导航栏后继续保持系统的 automatic 行为。
- 使用 iOS 27.1 SDK 编译并运行在 iOS 27.1 时，自定义栏会避让 active division / occlusion reserved region；旧系统继续使用当前 View 的 safe area。
- Pod 最低支持 iOS 16.0。TalkMe 等下游项目建议通过精确 commit 集成，并在自身业务页面完成旋转、多窗口、iPhone Duo 姿态与无障碍验收。
...
-|-|-
![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo1.gif) | ![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo2.gif) | ![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo3.gif) |

自定义titleView | 兼容scrollView横向滚动 |   可伸缩式导航栏
-|-|-
![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo4.gif) | ![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo8.gif) | ![](https://zxlee.oss-cn-shanghai.aliyuncs.com/github/ZXNavigationBar/ZXNavigationBarDemo6.gif) | 
