...
- 所有 UI 测试辅助功能文案使用英文；identifier 稳定且不参与多语言。旋转测试先发送 `XCUIDevice.orientation`；若 iPhone Duo 的真实 `UIWindowScene.effectiveGeometry` 与 window bounds 在有界等待内不变，只有测试进程精确读到 `XNAV_ALLOW_SCENE_ORIENTATION_FALLBACK=1` 时，才可通过 Demo fixture 调用公开 `requestGeometryUpdateWithPreferences:errorHandler:` 请求同一 scene 的目标方向。通过 `xcodebuild` 运行获批的 Duo 测试时，命令必须同时显式传入 `XNAV_ALLOW_SCENE_ORIENTATION_FALLBACK=1` 与 `TEST_RUNNER_XNAV_ALLOW_SCENE_ORIENTATION_FALLBACK=1`；后者仅负责把同一授权值转交 test runner，不是代码读取的第二个判定键。稳定 iOS 17 命令必须 `env -u` 这两个键，超时直接失败并保持 request count 为零。允许回退时也必须验证实际 scene/window 收敛并记录请求错误。禁止直接改 window/view frame；该回退不算 Device Hub 物理姿态证据。
